...
func ExampleClip_NewGetDurationCmd() {
	var clips []*ffcmd.Clip
	var cmds []string

	// Append the first command to the command slice.
	cmds = append(cmds, "total=0")

	// Create clips from clip data.
	for _, d := range clipData {
		c := ffcmd.NewClip(d.File, d.Start, d.End)
	...
	for i, c := range clips {
		...
		getDurationCmd, err := c.NewGetDurationCmd(varName, "")
		if err != nil {
			log.Printf("c.NewGetClipDurationCmd() error: %v", err)
			return
		}
...
func ExampleClip_NewCreateOneSubSRTCmd() {
	var cmds []string
	...
	for _, d := range clipData {
		...
		srtCmd, err := c.NewCreateOneSubSRTCmd(srtFile, d.Subtitle, "")
		if err != nil {
			log.Printf("c.NewCreateOneSubSRTCmd() error: %v", err)
			return
		}
		...
		rmSRTCmd, err := c.NewRemoveOneSubSRTCmd(srtFile)
		if err != nil {
			log.Printf("c.NewRemoveOneSubSRTCmd() error: %v", err)
			return
		}
	...
	// Output:
	// final command:
	// d=3.000 && d=$(printf "%.3f" $d) && sec=$(echo $d | awk -F. '{ print $1 }') && frac=$(echo $d | awk -F. '{ print substr($2, 1, 3) }') && hh=$((sec / 3600)) && mm=$((sec % 3600 / 60)) && ss=$((sec % 3600 % 60)) && printf -v end "%02d:%02d:%02d,%03d" $hh $mm $ss $frac && printf "1\n00:00:00,000 --> %s\nMido's tickling Mimao and he's enjoying..." $end > "01.srt" && cat 01.srt && echo && rm "01.srt" && d=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "02.MOV"); d=$(echo $d - 0.000 | bc) && d=$(printf "%.3f" $d) && sec=$(echo $d | awk -F. '{ print $1 }') && frac=$(echo $d | awk -F. '{ print substr($2, 1, 3) }') && hh=$((sec / 3600)) && mm=$((sec % 3600 / 60)) && ss=$((sec % 3600 % 60)) && printf -v end "%02d:%02d:%02d,%03d" $hh $mm $ss $frac && printf "1\n00:00:00,000 --> %s\nMimao's playing the toy." $end > "02.srt" && cat 02.srt && echo && rm "02.srt" && d=7.000 && d=$(printf "%.3f" $d) && sec=$(echo $d | awk -F. '{ print $1 }') && frac=$(echo $d | awk -F. '{ print substr($2, 1, 3) }') && hh=$((sec / 3600)) && mm=$((sec % 3600 / 60)) && ss=$((sec % 3600 % 60)) && printf -v end "%02d:%02d:%02d,%03d" $hh $mm $ss $frac && printf "1\n00:00:00,000 --> %s\nIt's hard to brush Maomi's teeth!" $end > "03.srt" && cat 03.srt && echo && rm "03.srt" && d=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "04.MOV"); d=$(echo $d - 2.000 | bc) && d=$(printf "%.3f" $d) && sec=$(echo $d | awk -F. '{ print $1 }') && frac=$(echo $d | awk -F. '{ print substr($2, 1, 3) }') && hh=$((sec / 3600)) && mm=$((sec % 3600 / 60)) && ss=$((sec % 3600 % 60)) && printf -v end "%02d:%02d:%02d,%03d" $hh $mm $ss $frac && printf "1\n00:00:00,000 --> %s\n\"What's this??!!\"
