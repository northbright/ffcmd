...
`
	...
	for i, c := range hl.Clips {
		...
		if c.Subtitle != "" {
			...
			createCmd, err := c.NewCreateOneSubSRTCmd(srtFile, c.Subtitle, c.Start, c.End)
			if err != nil {
...
`
	...
	for i, c := range hl.Clips {
		...
		if c.Subtitle != "" {
			...
			// Add command to create SRT file as ffmpeg's pre-commands(setup commmands).
			ffmpeg.AddPreCmd(createCmd)
	...
	getBGMFadeOutStartCmd := fmt.Sprintf("st=$(echo $%s - %d | bc) && st=$(printf \"%%.3f\" $st) && echo $st", totalDurationVarName, hl.BGMFadeOutDuration)
	ffmpeg.AddPreCmd(getBGMFadeOutStartCmd)

	bgm := ffcmd.NewFilterChain("[bgm]")
	bgm.AddInputByID(ffmpeg.AddInput(hl.BGM), "a", 0)

	//atrim := fmt.Sprintf("atrim=00:$%s,asetpts=PTS-STARTPTS", totalDurationVarName)
